 ...
 */
#warning "CommsDmx does not support this CPU"
#endif

void CommsDmx::setup(uint8_t pin, int maxChannels){
	this->usePin(pin);
	this->maxChannel(maxChannels);
}
 ...
 */
void CommsDmx::usePin(uint8_t pin) {
  dmxPin = pin;
 ...
 */
void CommsDmx::maxChannel(int channel) {
 ...
 */
void CommsDmx::write(int address, uint8_t value)
{
